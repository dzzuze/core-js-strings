...
function removeFirstOccurrences(str, value) {
  ...
  const beforeValue = str.substring(0, valueIndex);
  const afterValue = str.substring(valueIndex + value.length);
...
function sumOfCodes(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const valueCode = str.charCodeAt(i);
    sumArr += valueCode;
  }
  return sumArr;
}
...
function reverseString(str) {
  ...
  const revStr = strArr.reverse();
  const stringStr = revStr.join('');
  return stringStr;
}
...
function isPalindrome(str) {
  ...
  const revStrArr = Array.from(lowStr).reverse();
  const stringStrArr = revStrArr.join('');
  if (lowStr === stringStrArr) {
    return true;
  }
  return false;
}
...
function reverseWords(str) {
  ...
  const revWords = wordsArr.map((word) => word.split('').reverse().join(''));
...
function extractNameFromTemplate(value) {
  const firstChangeValue = value.replace('Hello, ', '');
  const secondChangeValue = firstChangeValue.replace('!', '');
  return secondChangeValue;
}
...
function unbracketTag(str) {
  const firstChangeStr = str.replace('<', '');
  const secondChangeStr = firstChangeStr.replace('>', '');
  return secondChangeStr;
}
...
function extractEmails(str) {
  ...
  const trimEmail = emailArr.map((email) => email.trim());
  return trimEmail;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      strArr[i] = String.fromCharCode(((charCode - 65 + 13) % 26) + 65);
    } else if (charCode >= 97 && charCode <= 122) {
      strArr[i] = String.fromCharCode(((charCode - 97 + 13) % 26) + 97);
    }
  }
  return strArr.join('');
}
...
function getCardId(value) {
  ...
  const cardNum = cardArr.indexOf(value);
  return cardNum;
}
